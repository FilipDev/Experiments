...
/** in other case you will see tons of warnings */
public class UnsafeImpl {
	...
	{
		return objectToMemoryMap(O, 256);
	}

	static public String objectToMemoryMap(Object obj, int length)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("Layout of @0x" + Long.toHexString(Object2ID(obj)*8));
		sb.append("\n");
		sb.append("     ");
		for (int x = 0; x <= 15; x++)
			sb.append(String.format("%1$02d", x, x) + " ");
		sb.append("\n");
		sb.append("\n");
		String hexByteString;
		for (long ct = 0; ct < length; ct++)
		{
			hexByteString = (ct % 16 == 15) ? "%2$02X" : "%2$02X ";
			hexByteString = (ct % 16 == 0) ? "%1$03d  %2$02X " : hexByteString;
			sb.append(String.format(hexByteString, ct, unsafe.getByte(obj, ct)));
			if (ct % 16 == 15 && ct != length - 1)
			{
				sb.append("\n");
			}
		}
	...
	static public Field fetchField(Class Source, String... Names)
	{
	...
	final static String asdf = "interesting.";
	...
	/*static*/ public void main(String[] args) throws Throwable
	{
		...
		for (int x = 0; x <= 5000; x++)
			try
			{
				System.out.println(x + " " + cp.getStringAt(x));
			} catch (Exception e)
			{
			}
